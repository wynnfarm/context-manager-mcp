// Enhanced Context Manager Dashboard - Browser Compatible Version
// Safe enhancement - no MCP protocol impact

// Use global React and Chart.js from CDN
const { useState, useEffect, useRef, useCallback, useMemo } = React;
const Chart = window.Chart;

// Enhanced WebSocket hook with connection timing fix
const useWebSocket = (url, onMessage) => {
  const [status, setStatus] = useState({ connected: false, text: "Connecting...", error: null });
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      setStatus({ connected: false, text: "Connecting...", error: null });
      ...
      wsRef.current.onopen = () => {
        ...
        clearTimeout(connectionTimeoutRef.current);
        setStatus({ connected: true, text: "Connected", error: null });
      ...
      wsRef.current.onclose = (event) => {
        ...
        clearTimeout(connectionTimeoutRef.current);
        setStatus({ connected: false, text: "Disconnected", error: null });
        isConnectingRef.current = false;
      ...
      wsRef.current.onerror = (error) => {
        ...
        clearTimeout(connectionTimeoutRef.current);
        setStatus({ connected: false, text: "Error", error: error.message });
        isConnectingRef.current = false;
      };
    } catch (error) {
      console.error("Failed to setup WebSocket:", error);
      clearTimeout(connectionTimeoutRef.current);
      setStatus({ connected: false, text: "Failed to connect", error: error.message });
      isConnectingRef.current = false;
    }
  }, [url, onMessage]);
  ...
  }, []); // Empty dependencies to prevent re-renders
      ...
      <div className="space-y-2">
        {projects.map((project) => (
          <div key={project.name} className="border border-gray-200 rounded-lg overflow-hidden">
            ...
            <div
              className={`transition-all duration-300 overflow-hidden ${
                expandedProject === project.name ? "max-h-96" : "max-h-0"
              }`}
            >
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mt-8">
      <div className="flex items-center justify-between mb-4">
        ...
        <button className="text-blue-600 hover:text-blue-700 text-sm flex items-center" onClick={onViewAll}>
          <i className="fas fa-expand-alt mr-1"></i>
          View All
        </button>
      </div>
...
const SlidePanel = ({ isOpen, onClose, title, children, zIndex = "z-40" }) => (
  <>
    <div
    ...
    />
    <div
      className={`fixed top-0 right-0 w-full max-w-2xl h-full bg-white shadow-xl transition-transform duration-300 ${zIndex} ${
        isOpen ? "translate-x-0" : "translate-x-full pointer-events-none"
      }`}
    >
      <div className="flex items-center justify-between p-6 border-b border-gray-200 relative z-60">
        <h3 className="text-xl font-semibold text-gray-900">{title}</h3>
        <button
          onClick={onClose}
          className="text-gray-400 hover:text-gray-600 p-2 rounded-lg hover:bg-gray-100 relative z-60"
        ...
        <button
          onClick={onClose}
          className="ml-2 px-3 py-1 text-sm bg-gray-100 hover:bg-gray-200 text-gray-700 rounded relative z-60"
        >
          Close
        </button>
      </div>
...
const AdvancedSearchPanel = ({ isOpen, onClose, onSearch, onClear }) => {
  ...
  return (
    <SlidePanel isOpen={isOpen} onClose={onClose} title="Advanced Search" zIndex="z-50">
      <div className="space-y-4">
        ...
        <div className="pt-4 border-t border-gray-200">
          <button onClick={onClose} className="w-full px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200">
            Close Panel
          </button>
        </div>
      </div>
...
const Dashboard = () => {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      if (!result.success) {
        throw new Error(result.message || "Failed to load context data");
      }

      // Transform context data into analytics format
      const contexts = result.data || [];
      const analyticsData = {
        overall_metrics: {
          total_projects: contexts.length,
          projects_with_issues: contexts.filter((c) => c.pending_issues_count > 0).length,
          projects_with_goals: contexts.filter((c) => c.current_goal).length,
          average_completion:
            contexts.length > 0
              ? Math.round(
                  (contexts.reduce((sum, c) => sum + (c.completed_features_count || 0), 0) / contexts.length) * 10,
                )
              : 0,
        },
        project_summaries: contexts.map((context) => ({
          name: context.name,
          completion: Math.round(
            ((context.completed_features_count || 0) / Math.max(context.features?.length || 1, 1)) * 100,
          ),
          health: context.status === "active" ? 85 : context.status === "completed" ? 100 : 60,
          status: context.status,
          features: context.features || [],
          completed_features: context.completed_features_count || 0,
          pending_issues: context.pending_issues_count || 0,
        })),
      };
  ...
  const handleWebSocketMessage = useCallback(
    (data) => {
      ...
      } else if (data.type === "heartbeat" || data.type === "heartbeat_response") {
        console.log("Heartbeat received:", data.type);
      } else if (
        data.type === "context_updated" ||
        data.type === "feature_completed" ||
        data.type === "issue_resolved"
      ) {
        console.log("Context update received, reloading data...");
        loadData();
      } else {
  ...
  // Add keyboard shortcut to close panels
  useEffect(() => {
    const handleKeyDown = (event) => {
      if (event.key === "Escape") {
        setAdvancedSearchOpen(false);
        setSlidePanel({ isOpen: false, type: null, title: "", content: null });
      }
    };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <SearchAndFilter onSearch={handleSearch} onFilter={handleFilter} onAdvancedSearch={handleAdvancedSearch} />
        ...
        <ProjectAccordion projects={data.project_summaries} onProjectAction={handleProjectAction} />

        <InsightsSection metrics={data.overall_metrics} onViewAll={handleViewAllInsights} />
      </main>
...
// Make Dashboard globally available for browser use
